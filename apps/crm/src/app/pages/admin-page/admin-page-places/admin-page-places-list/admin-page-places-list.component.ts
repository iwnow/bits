...
@Component({
  selector: 'b-admin-page-places-list',
  templateUrl: './admin-page-places-list.component.html',
  styleUrls: ['./admin-page-places-list.component.css'],
  standalone: true,
  imports: [CommonModule, BitsGridComponent, FrmsComponent, DialogModule],
})
export class AdminPagePlacesListComponent implements OnInit {
  ...
  createGridOption(): BGridOptions {
    const opt: BGridOptions = {
      getOptions: () => {
        return {
          columnDefs: columnsFromClass(DOMAIN.CompanyPlace),
          rowSelection: 'single',
          onSelectionChanged: () => {
